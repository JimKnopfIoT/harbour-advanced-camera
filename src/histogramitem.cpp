...
void HistogramItem::updateFromGrab(QObject *grabResult)
{
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            ...
            // Skip the viewfinder's letterbox bars: pure black (exactly 0,0,0).
            // Real image shadows carry sensor noise and are virtually never
            // exactly black, so this drops the bars without losing true darks.
            if ((r | g | b) == 0)
                continue;
    ...
    const int total = qMax(1, counted);
